<div class="row" id="js-trigger-invoices-modal-add-edit" data-payload="{{ $page['section'] ?? '' }}">
    <div class="col-lg-12">
        ...
        <div class="modal-meta-data">
            <small><strong>{{ cleanLang(__('lang.created_by')) }}:</strong> {{ $invoice->first_name }} {{ $invoice->last_name}} |
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-12 col-lg-9">
                ...
                <select name="bill_clientid" id="bill_clientid"
                    ...
                    <option value="{{ $invoice->bill_clientid ?? '' }}">{{ $invoice->client_company_name }}</option>
                    @endif
                    <!--creating invoice from an expense-->
                    @if(config('visibility.invoice_from_expense_client_name'))
                    <option value="{{ $expense->expense_clientid ?? '' }}">{{ $expense->client_company_name }}</option>
                    @endif
                </select>
            </div>
        </div>
        ...
        <div class="hidden" id="edit_bill_recurring_toggle">
            ...
            <!-- terms-->
            <div class="form-group row">
                <label class="col-12 text-left control-label col-form-label">{{ cleanLang(__('lang.terms_and_conditions')) }}</label>
                <div class="col-12">
