...
class PDFResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (request('view') == 'preview') {
            ...
            return view('pages/bill/bill-pdf', compact('page', 'bill', 'taxrates', 'taxes', 'elements', 'lineitems'))->render();
        }

        //download pdf view
        config(['css.bill_mode' => 'pdf-mode-download']);
        $pdf = PDF::loadView('pages/bill/bill-pdf', compact('page', 'bill', 'taxrates', 'taxes', 'elements', 'lineitems'));
