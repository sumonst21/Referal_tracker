...
/** --------------------------------------------------------------------------------
 * This classes renders the response for the [index] process for the goals
 * controller
 * @package    Grow CRM
 * @author     NextLoop
 *----------------------------------------------------------------------------------*/
...
class IndexResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            case 'load':
                $template = 'pages/goals/components/table/ajax';
                $dom_container = '#goals-td-container';
                $dom_action = 'append';
                break;
            ...
            case 'search':
                $template = 'pages/goals/components/table/table';
                $dom_container = '#goals-table-wrapper';
                $dom_action = 'replace-with';
                break;

            //template and dom - for ajax initial loading
            default:
                $template = 'pages/goals/tabswrapper';
            ...
            if ($goals->currentPage() < $goals->lastPage()) {
                $url = loadMoreButtonUrl($goals->currentPage() + 1, request('source'));
            ...
            $html = view($template, compact('page', 'goals'))->render();
            ...
            $jsondata['dom_html'][] = [
                ...
                'action' => 'replace',
                'value' => strtoupper(__('lang.goals')),
            ];
            ...
            //standard view
            $page['url'] = loadMoreButtonUrl($goals->currentPage() + 1, request('source'));
            $page['loading_target'] = 'goals-td-container';
            $page['visibility_show_load_more'] = ($goals->currentPage() < $goals->lastPage()) ? true : false;
            return view('pages/goals/wrapper', compact('page', 'goals'))->render();
        }
