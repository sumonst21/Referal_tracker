...
/** --------------------------------------------------------------------------------
 * This classes renders the response for the [index] process for the reminders
 * controller
 * @package    Grow CRM
 * @author     NextLoop
 *----------------------------------------------------------------------------------*/
...
class IndexResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            case 'load':
                $template = 'pages/reminders/components/table/ajax';
                $dom_container = '#reminders-td-container';
                $dom_action = 'append';
                break;
            ...
            case 'search':
                $template = 'pages/reminders/components/table/table';
                $dom_container = '#reminders-table-wrapper';
                $dom_action = 'replace-with';
                break;

            //template and dom - for ajax initial loading
            default:
                $template = 'pages/reminders/tabswrapper';
            ...
            if ($reminders->currentPage() < $reminders->lastPage()) {
                $url = loadMoreButtonUrl($reminders->currentPage() + 1, request('source'));
            ...
            $html = view($template, compact('page', 'reminders'))->render();
            ...
            $jsondata['dom_html'][] = [
                ...
                'action' => 'replace',
                'value' => strtoupper(__('lang.reminders')),
            ];
            ...
            //standard view
            $page['url'] = loadMoreButtonUrl($reminders->currentPage() + 1, request('source'));
            $page['loading_target'] = 'reminders-td-container';
            $page['visibility_show_load_more'] = ($reminders->currentPage() < $reminders->lastPage()) ? true : false;
            return view('pages/reminders/wrapper', compact('page', 'reminders'))->render();
        }
