...
use App\Http\Requests\Client_Tasks\TaskStoreUpdate;
use App\Http\Responses\Client_Tasks\AttachFilesResponse;
use App\Http\Responses\Client_Tasks\ArchiveResponse;
use App\Http\Responses\Client_Tasks\ActivateResponse;
use App\Http\Responses\Client_Tasks\ChecklistResponse;
use App\Http\Responses\Client_Tasks\CreateResponse;
use App\Http\Responses\Client_Tasks\DestroyResponse;
use App\Http\Responses\Client_Tasks\IndexKanbanResponse;
use App\Http\Responses\Client_Tasks\IndexListResponse;
use App\Http\Responses\Client_Tasks\ShowResponse;
use App\Http\Responses\Client_Tasks\StoreChecklistResponse;
use App\Http\Responses\Client_Tasks\StoreCommentResponse;
use App\Http\Responses\Client_Tasks\StoreResponse;
use App\Http\Responses\Client_Tasks\TimerStartResponse;
use App\Http\Responses\Client_Tasks\TimerStopResponse;
use App\Http\Responses\Client_Tasks\UpdateChecklistResponse;
use App\Http\Responses\Client_Tasks\UpdateErrorResponse;
use App\Http\Responses\Client_Tasks\UpdateResponse;
use App\Http\Responses\Client_Tasks\updateCustomFieldsResponse;
use App\Http\Responses\Client_Tasks\UpdateStatusResponse;
...
use App\Repositories\C_TaskRepository;
...
class Client_Tasks extends Controller {

    /**
     * The task repository instance.
     */
    protected $c_taskrepo;
    ...
    public function __construct(
        C_TaskRepository $c_taskrepo,
        TagRepository $tagrepo,
        UserRepository $userrepo,
        TimerRepository $timerrepo,
        C_TaskPermissions $c_taskpermissions,
        ...
        $this->c_taskrepo = $c_taskrepo;
        $this->tagrepo = $tagrepo;
        $this->userrepo = $userrepo;
        $this->c_taskpermissions = $c_taskpermissions;
        $this->c_taskmodel = $c_taskmodel;
        ...
        //route middleware
        $this->middleware('clienttasksMiddlewareTimer')->only([
            'timerStart',
            'timerStop',
            'timerStopAll',
        ]);
        ...
        $this->middleware('clienttasksMiddlewareCreate')->only([
            'create',
            'store',
        ]);

        $this->middleware('clienttasksMiddlewareShow')->only([
            'show',
        ]);

        $this->middleware('clienttasksMiddlewareEdit')->only([
            'updateDescription',
            'updateTitle',
            'updateStartDate',
            'updateDueDate',
        ...
        $this->middleware('clienttasksMiddlewareParticipate')->only([
            'storeComment',
            'attachFiles',
        ]);

        $this->middleware('clienttasksMiddlewareDeleteAttachment')->only([
            'deleteAttachment',
        ]);

        $this->middleware('clienttasksMiddlewareDownloadAttachment')->only([
            'downloadAttachment',
        ]);

        $this->middleware('clienttasksMiddlewareDeleteComment')->only([
            'deleteComment',
        ]);

        $this->middleware('clienttasksMiddlewareEditDeleteChecklist')->only([
            'updateChecklist',
            'deleteChecklist',
            'toggleChecklistStatus',
        ]);

        $this->middleware('clienttasksMiddlewareDestroy')->only([
            'destroy',
        ]);

        $this->middleware('clienttasksMiddlewareAssign')->only([
            'updateAssigned',
        ]);
    }
    ...
    public function create(CategoryRepository $categoryrepo) {
        ...
        $payload = [
            ...
            'milestones' => $milestones,
            'stats' => $this->statsWidget(),
            'fields' => \App\Models\CustomField::Where('customfields_type', 'c_tasks')->where('customfields_status', 'enabled')->get(),
        ];
    ...
    public function customFieldValidationFailed() {
        ...
        //custom field validation
        $fields = \App\Models\CustomField::Where('customfields_type', 'c_tasks')->get();
        $errors = '';
    ...
    public function show(
        ...
        //get the task
        $tasks = $this->c_taskrepo->search($id);
        ...
        $payload = [
            ...
            'fields' => \App\Models\CustomField::Where('customfields_type', 'tasks')->where('customfields_status', 'enabled')->get(),
        ];
    ...
    public function updateCustomFields($id) {
        ...
        //process reponse
        return new updateCustomFieldsResponse($payload);
    }
    ...
    private function statsWidget($data = array()) {
        ...
        $count_in_progress = $this->c_taskrepo->search('', ['stats' => 'count-in-progress']);
        $count_testing = $this->c_taskrepo->search('', ['stats' => 'count-testing']);
        $count_awaiting_feedback = $this->c_taskrepo->search('', ['stats' => 'count-awaiting-feedback']);
        $count_completed = $this->c_taskrepo->search('', ['stats' => 'count-completed']);
