...
namespace App\Http\Middleware\Client_Tasks;
...
class Index {
     ...
     *
     */
    public function __construct(ProjectPermissions $projectpermissons, C_TaskPermissions $c_taskpermissions) {
    ...
    public function handle($request, Closure $next) {
        ...
        if (is_numeric(request()->route('task')) && request()->segment(2) == 'v') {
            if ($this->c_taskpermissions->check('view', request()->route('task'))) {
                config([
