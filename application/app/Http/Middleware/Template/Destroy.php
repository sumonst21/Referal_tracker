...
class Destroy {
    ...
    public function handle($request, Closure $next) {
        ...
        if (is_array(request('ids'))) {
            ...
            Log::error("no items were sent with this request", ['process' => '[permissions][foos][action-bar]', 'ref' => config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__, 'foo id' => $foo_id ?? '']);
            abort(409);
        }
