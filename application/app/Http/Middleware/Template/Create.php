...
class Create {
    ...
    public function handle($request, Closure $next) {
        ...
        //permission denied
        Log::error("permission denied", ['process' => '[permissions][foos][create]', 'ref' => config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__]);
        abort(403);
    }
        ...
        if (request()->filled('fooresource_id') && request()->filled('fooresource_type')) {
            ...
            if (request('fooresource_type') == 'project') {
                if ($project = \App\Models\Project::Where('project_id', request('fooresource_id'))->first()) {
                    ...
                    //error not found
                    Log::error("the resource project could not be found", ['process' => '[permissions][foos][create]', 'ref' => config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__]);
                    abort(404);
                }
            }
