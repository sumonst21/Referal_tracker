...
class C_TaskRepository {
     ...
     * Inject dependecies
     */
    public function __construct(C_Task $tasks) {
        $this->tasks = $tasks;
    }
    ...
    public function search($id = '', $data = []) {
        ...
        //joins
        $tasks->leftJoin('projects', 'projects.project_id', '=', 'c__tasks.task_projectid');
        $tasks->leftJoin('milestones', 'milestones.milestone_id', '=', 'c__tasks.task_milestoneid');
        $tasks->leftJoin('users', 'users.id', '=', 'c__tasks.task_creatorid');
        ...
        $tasks->selectRaw('(SELECT COUNT(*)
                                      ...
                                      AND events.event_parent_id = c__tasks.task_id
        ...
        $tasks->selectRaw('(SELECT COALESCE(SUM(timer_time), 0)
                                           FROM timers WHERE timer_taskid = c__tasks.task_id)
                                           AS sum_all_time');
        ...
        if (in_array(request('sortorder'), array('desc', 'asc')) && request('orderby') != '') {
            //direct column name
            if (Schema::hasColumn('c__tasks', request('orderby'))) {
    ...
    public function create($position = '') {
        ...
        if (!is_numeric($position)) {
            Log::error("validation error - invalid params", ['process' => '[C_TaskRepository]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__]);
            return false;
        }
        ...
        if ($task->save()) {
            ...
            Log::error("record could not be saved - database error", ['process' => '[C_TaskRepository]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__]);
            return false;
        }
    }
    ...
    public function timerStop($id) {
        ...
        if ($item->save()) {
            ...
            Log::error("record could not be updated - database error", ['process' => '[C_TaskRepository]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__]);
            return false;
        }
    }
