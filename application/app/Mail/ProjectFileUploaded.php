...
/** --------------------------------------------------------------------------------
 * This classes renders the [project file] email and stores it in the queue
 * @package    Grow CRM
 * @author     NextLoop
 *----------------------------------------------------------------------------------*/
...
class ProjectFileUploaded extends Mailable {
    use Queueable;
     ...
     * @return void
     */
    public function __construct($user = [], $file = [], $obj = []) {
    ...
    public function build() {
        ...
        //email template
        if (!$template = \App\Models\EmailTemplate::Where('emailtemplate_name', 'Project File Uploaded')->first()) {
            return false;
        }

        //validate
        if (!$this->obj instanceof \App\Models\Project || !$this->user instanceof \App\Models\User || !$this->file instanceof \App\Models\File) {
            return false;
        }
        ...
        $payload += [
            ...
            'file_name' => $this->file->file_filename,
            'file_size' => $this->file->file_size,
            'file_extension' => $this->file->file_extension,
            'file_url' => url('files/download?file_id=' . $this->file->file_uniqueid),
            'uploader_first_name' => $this->file->first_name,
            'uploader_last_name' => $this->file->last_name,
            ...
            'project_files_url' => url('/projects/' . $this->obj->project_id . '/files'),
        ];
