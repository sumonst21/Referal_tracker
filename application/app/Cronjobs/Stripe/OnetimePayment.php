...
class OnetimePayment {
    ...
    public function __invoke(
        ...
        if ($webhooks = \App\Models\Webhook::Where('webhooks_gateway_name', 'stripe')
            ->where('webhooks_payment_type', 'onetime')
            ->where('webhooks_status', 'new')->take($limit)->get()) {

            //log that its run
            Log::info("found applicable webhooks", ['process' => '[cronjob][stripe-onetime-payment]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__, 'payload' => $webhooks]);
            ...
            foreach ($webhooks as $webhook) {
                ...
                Log::info("stripe cronjob [onetime] - found valid webhook to process", ['process' => '[cronjob][stripe-onetime-payment]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__, 'webhook' => $webhook]);
                ...
                if (!$invoice = \App\Models\Invoice::Where('bill_invoiceid', $session->session_invoices)->first()) {
                    //log error
                    Log::critical("no corresponding (invoice) (Invoice ID: $session->session_invoices) record was found for this payment session", ['process' => '[stripe-cron]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__, 'payment_session' => $session]);
                    $webhook->update([
                        'webhooks_status' => 'failed',
