...
/** ---------------------------------------------------------------------------------------------------
 * [Razorpay Complete Onetime]
 * This cronjob checks for recorded webhooks from razorpay and does the following
...
namespace App\Cronjobs\Razorpay;
...
class OnetimePayment {
    ...
    public function __invoke(
        ...
        $limit = 2;
        if ($webhooks = \App\Models\Webhook::Where('webhooks_gateway_name', 'razorpay')
            ...
            foreach ($webhooks as $webhook) {
                ...
                if (!$invoice = \App\Models\Invoice::Where('bill_invoiceid', $session->session_invoices)->first()) {
                    //log error
                    Log::critical("no corresponding (invoice) (Invoice ID: $session->session_invoices) record was found for this payment session", ['process' => '[razorpay-cron]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__, 'payment_session' => $session]);
                    $webhook->update([
                        'webhooks_status' => 'failed',
